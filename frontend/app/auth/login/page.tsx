...
import { Lock, Eye, EyeOff, AlertCircle, User, X } from "lucide-react";
...
export function LoginForm({ onSuccess, onNavigate, onClose }: LoginFormProps) {
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-2xl shadow-xl px-8 py-10 relative">
      ...
      <p className="text-sm text-gray-500 mb-8">
        Chưa có tài khoản?{" "}
        {onNavigate
          ? <button onClick={() => onNavigate("register")} className="text-[#c8a46b] hover:underline font-medium">Tạo tài khoản</button>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          {onNavigate
            ? <button type="button" onClick={() => onNavigate("forgot")} className="text-[#c8a46b] hover:underline">Quên mật khẩu?</button>
